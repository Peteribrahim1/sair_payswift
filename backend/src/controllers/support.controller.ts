...
You must respond STRICTLY with a single JSON object in this format:
{
  ...
  "action": {
    ...
    if (text.includes('balance') || text.includes('wallet') || text.includes('money') || text.includes('funds')) {
      ...
      // Detect network
      if (text.includes('mtn')) payload.network = 'MTN';
      else if (text.includes('airtel')) payload.network = 'AIRTEL';
      else if (text.includes('glo')) payload.network = 'GLO';
      else if (text.includes('9mobile')) payload.network = '9MOBILE';
      // Detect amount
